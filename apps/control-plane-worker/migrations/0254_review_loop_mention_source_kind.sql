-- Widen pr_review_response_epochs.source_kind CHECK to include 'mention'.
-- SQLite cannot alter a CHECK constraint in place; rebuild the table (same pattern as 0122/0156/0225).
-- @arcanist mention epochs are dedicated review-loop turns triggered by a user @-mention (top-level
-- PR comment, review-comment reply, or review body) and must dispatch regardless of manual review mode.
-- They stay a distinct class from bot, human, CI, verification, and merge_conflict epochs on the same head.
-- Schema below reproduces the table exactly as of 0225 (0243 was a data-only UPDATE); only the CHECK differs.
CREATE TABLE pr_review_response_epochs_new (
  ...
  source_kind TEXT NOT NULL DEFAULT 'bot' CHECK (source_kind IN ('bot', 'human', 'mixed', 'ci', 'verification', 'merge_conflict', 'mention')),
  prompted_source_ids_json TEXT NOT NULL DEFAULT '[]',
  carried_forward_source_ids_json TEXT NOT NULL DEFAULT '[]',
  carry_forward_no_progress_count INTEGER NOT NULL DEFAULT 0
);
...
INSERT INTO pr_review_response_epochs_new
  (id, session_id, owner_user_id, repo_owner, repo_name, pr_number, pr_url, head_sha, wave,
   ...
   carried_forward_source_ids_json, carry_forward_no_progress_count)
