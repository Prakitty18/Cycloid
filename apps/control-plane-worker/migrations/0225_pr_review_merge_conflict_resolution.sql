...
ADD COLUMN merge_conflict_resolution_enabled INTEGER NOT NULL DEFAULT 0;

-- Widen pr_review_response_epochs.source_kind CHECK to include 'merge_conflict'.
-- SQLite cannot alter a CHECK constraint in place; rebuild the table (same pattern as 0122/0156).
-- Merge-conflict epochs are dedicated review-loop turns for dirty PR heads and must stay distinct from
-- bot, human, CI, and verification epochs on the same head.
CREATE TABLE pr_review_response_epochs_new (
  ...
  source_kind TEXT NOT NULL DEFAULT 'bot' CHECK (source_kind IN ('bot', 'human', 'mixed', 'ci', 'verification', 'merge_conflict')),
