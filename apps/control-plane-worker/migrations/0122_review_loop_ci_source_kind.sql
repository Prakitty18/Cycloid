-- Widen pr_review_response_epochs.source_kind CHECK to include 'ci'.
-- SQLite cannot alter a CHECK constraint in place; rebuild the table.
-- The 0119 migration added source_kind via ADD COLUMN ... CHECK (source_kind IN ('bot','human','mixed')).
-- CI-failure epochs need source_kind = 'ci' (review-loop v1.2).
CREATE TABLE pr_review_response_epochs_new (
  ...
  source_kind TEXT NOT NULL DEFAULT 'bot' CHECK (source_kind IN ('bot', 'human', 'mixed', 'ci'))
);
...
INSERT INTO pr_review_response_epochs_new
  (id, session_id, owner_user_id, repo_owner, repo_name, pr_number, pr_url, head_sha, wave,
   ...
   last_error, created_at, updated_at, source_kind)
