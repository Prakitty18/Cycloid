...
MESSAGE="${*:-Cycloid is currently degraded.}"
if [[ ${#MESSAGE} -gt 1000 ]]; then
  echo "Status message must be 1,000 characters or fewer; got ${#MESSAGE}." >&2
  exit 1
fi
...
STATUS_STATE="down" STATUS_MESSAGE="$MESSAGE" node --input-type=module - "$TMP_FILE" <<'NODE'
